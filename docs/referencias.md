# Referências Bibliográficas

Herdadas do projeto original `deep-acordao-tcu`, agrupadas por domínio.
Todas foram citadas em artefatos científicos e mantêm-se pertinentes ao pipeline v2.

---

## Domínio A — Jurimetria Preditiva e Controle Externo

BRASIL. Tribunal de Contas da União. **Portal de Dados Abertos do TCU: Acórdãos Completos**. Brasília: TCU, 2024. Disponível em: <https://sites.tcu.gov.br/dados-abertos/jurisprudencia/>. Acesso em: 03 jul. 2026.

TVEITA, Sondre; HUSTAD, Eli. **Benefits and challenges of AI in the public sector**. In: HAWAII INTERNATIONAL CONFERENCE ON SYSTEM SCIENCES, 58., 2025, Maui. *Proceedings…* Honolulu: University of Hawaii, 2025. p. 1–10. *(HICSS — anais)*

ALETRAS, Nikolaos et al. **Predicting judicial decisions of the European Court of Human Rights: A Natural Language Processing perspective**. *PeerJ Computer Science*, v. 2, e93, 2016.
*(Periódico — PeerJ Computer Science, ISSN 2376-5992, open access)*
Disponível em: <https://doi.org/10.7717/peerj-cs.93>. Acesso em: 03 jul. 2026.

MEDVEDEVA, Masha; VOLS, Michel; WIELING, Martijn. **Using machine learning to predict decisions of the European Court of Human Rights**. *Artificial Intelligence and Law*, v. 28, n. 2, p. 237–266, 2020.
*(Periódico — Artificial Intelligence and Law, Springer)*
Disponível em: <https://doi.org/10.1007/s10506-019-09255-y>.

LAGE-FREITAS, André et al. **Predicting Brazilian court decisions**. *PeerJ Computer Science*, v. 8, e904, 2022.
*(Periódico — PeerJ Computer Science, open access)*
Disponível em: <https://doi.org/10.7717/peerj-cs.904>.

---

## Domínio B — Técnica: Modelos de Linguagem e NLP Jurídico

VASWANI, Ashish et al. **Attention Is All You Need**. In: ADVANCES IN NEURAL INFORMATION PROCESSING SYSTEMS, 30., 2017, Long Beach. *Proceedings…* Red Hook: Curran Associates, 2017. p. 5998–6008.
*(NeurIPS — anais)* Disponível em: <https://arxiv.org/abs/1706.03762>.

DEVLIN, Jacob et al. **BERT: Pre-training of Deep Bidirectional Transformers for Language Understanding**. In: CONFERENCE OF THE NORTH AMERICAN CHAPTER OF THE ASSOCIATION FOR COMPUTATIONAL LINGUISTICS, 2019, Minneapolis. *Proceedings…* Stroudsburg: ACL, 2019. p. 4171–4186.
*(NAACL-HLT — anais)* Disponível em: <https://arxiv.org/abs/1810.04805>.

KIM, Yoon. **Convolutional Neural Networks for Sentence Classification**. In: CONFERENCE ON EMPIRICAL METHODS IN NATURAL LANGUAGE PROCESSING, 2014, Doha. *Proceedings…* Stroudsburg: ACL, 2014. p. 1746–1751.
*(EMNLP — anais)* Disponível em: <https://arxiv.org/abs/1408.5882>.

SOUZA, Fábio; NOGUEIRA, Rodrigo; LOTUFO, Roberto. **BERTimbau: Pretrained BERT Models for Brazilian Portuguese**. In: INTELLIGENT SYSTEMS — BRACIS, 9., 2020, Rio Grande. *Proceedings…* Cham: Springer, 2020. p. 403–417.
*(BRACIS — anais)* Disponível em: <https://link.springer.com/chapter/10.1007/978-3-030-61377-8_28>.

DOMINGUES, Luciano. **legal-bert-base-cased-ptbr: BERT model pre-trained on Brazilian legal corpus**. HuggingFace Hub, 2022.
*(Modelo público)* Disponível em: <https://huggingface.co/dominguesm/legal-bert-base-cased-ptbr>.

WOLF, Thomas et al. **Transformers: state-of-the-art natural language processing**. In: CONFERENCE ON EMPIRICAL METHODS IN NATURAL LANGUAGE PROCESSING: SYSTEM DEMONSTRATIONS, 2020, online. *Proceedings…* Stroudsburg: ACL, 2020. p. 38–45.
*(EMNLP System Demos — anais)* Disponível em: <https://arxiv.org/abs/1910.03771>.

---

## Domínio C — Adaptação, Otimização, Desbalanceamento e Explicabilidade

HU, Edward J. et al. **LoRA: Low-Rank Adaptation of Large Language Models**. In: INTERNATIONAL CONFERENCE ON LEARNING REPRESENTATIONS, 2022, online. *Proceedings…* [S.l.]: OpenReview, 2022.
*(ICLR — anais)* Disponível em: <https://arxiv.org/abs/2106.09685>.

SUN, Chi; QIU, Xipeng; XU, Yuanbin; HUANG, Xuanjing. **How to Fine-Tune BERT for Text Classification?** In: CHINESE COMPUTATIONAL LINGUISTICS, 18., 2019, Kunming. *Proceedings…* Cham: Springer, 2019. p. 194–206.
*(CCL — anais)* Disponível em: <https://arxiv.org/abs/1905.05583>.

LIN, Tsung-Yi et al. **Focal Loss for Dense Object Detection**. In: IEEE INTERNATIONAL CONFERENCE ON COMPUTER VISION, 2017, Veneza. *Proceedings…* [S.l.]: IEEE, 2017. p. 2980–2988.
*(ICCV — anais — Focal Loss para desbalanceamento severo)* Disponível em: <https://arxiv.org/abs/1708.02002>.

KING, Gary; ZENG, Langche. **Logistic Regression in Rare Events Data**. *Political Analysis*, v. 9, n. 2, p. 137–163, 2001.
*(Periódico — Cambridge University Press, tratamento de eventos raros e ponderação inversa)*
Disponível em: <https://doi.org/10.1093/oxfordjournals.pan.a004868>.

PEDREGOSA, Fabian et al. **Scikit-learn: machine learning in Python**. *Journal of Machine Learning Research*, v. 12, p. 2825–2830, 2011.
*(JMLR — periódico, open access — `class_weight='balanced'`)*
Disponível em: <https://jmlr.org/papers/v12/pedregosa11a.html>.

RIBEIRO, Marco Tulio; SINGH, Sameer; GUESTRIN, Carlos. **"Why should I trust you?": explaining the predictions of any classifier**. In: ACM SIGKDD INTERNATIONAL CONFERENCE ON KNOWLEDGE DISCOVERY AND DATA MINING, 22., 2016, San Francisco. *Proceedings…* New York: ACM, 2016. p. 1135–1144.
*(KDD — anais)* Disponível em: <https://arxiv.org/abs/1602.04938>.
